defmodule BasicLiveviewWeb.HomeLive do
  alias BasicLiveview.Posts
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(BasicLiveview.PubSub, "posts")
    ...
    |> case do
      {:ok, post} ->
        socket =
          ...
          Phoenix.PubSub.broadcast(BasicLiveview.PubSub, "posts", {:new, Map.put(post, :user, user)})
        {:noreply, socket}
  ...
  defp consume_files(socket) do
    consume_uploaded_entries(socket, :image, fn %{path: path}, _entry ->
      dest =
        Path.join([:code.priv_dir(:basic_liveview), "static", "uploads", Path.basename(path)])
